...
--Draw Text and it's background
  ...
  v_graph.widget.image = v_graph_surface
...
local function update_master(v_graph)
    ...
    data[v_graph].mastertimer:connect_signal("timeout", function() 
...
function new(args)
    ...
    local v_graph = {}
    v_graph.widget = imagebox(args)
    v_graph.widget:set_resize(false)
