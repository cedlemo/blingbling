return
{
  superproperties = require("blingbling.superproperties");
  helpers = require("blingbling.helpers");
  value_text_box = require("blingbling.value_text_box");
  --text_box =require("blingbling.text_box");
  line_graph = require("blingbling.line_graph");
  progress_graph = require("blingbling.progress_graph");
  triangular_progressgraph = require("blingbling.triangular_progress_graph");
  volume = require("blingbling.volume");
  --layout = require("blingbling.wibox.layout");
  --flex = require("blingbling.wibox.layout.flex");
  --array = require("blingbling.wibox.layout.array");
  --calendar = require("blingbling.calendar");
  --udisks_glue = require("blingbling.udisks_glue");
  --system = require("blingbling.system");
  --Graphical widget library for Awesome Windows Manager.
  --module("blingbling")
}

--TODO adapt net.lua
--TODO clean new functions
--TODO check utility of group in add_value
--TODO system.lua
--TODO udisks_glue.lua

